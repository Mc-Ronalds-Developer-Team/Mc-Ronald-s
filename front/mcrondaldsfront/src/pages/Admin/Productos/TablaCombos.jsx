...
const TablaCombos = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const comboCat = categories.find(c => c.name.toLowerCase() === CATEGORY_NAME.toLowerCase());

        if (comboCat) {
          setCategoryId(comboCat.idCategory);
          const items = await api.getProductsByCategoryId(comboCat.idCategory);
          setProductos(items);
          setLoading(false);
        } else {
          setError("Categoría 'Combos' no encontrada en el backend.");
          setLoading(false);
        }
      } catch (err) {
        console.error(err);
        // Si falla, quizás el backend no está corriendo o no hay datos iniciales
        setError("Error conectando con el servidor. Asegúrese que el backend está corriendo.");
        setLoading(false);
      }
    };
    loadData();
  ...
  if (loading) return <div className="admin-content"><p>Cargando datos del servidor...</p></div>;
  if (error) return <div className="admin-content"><p style={{ color: "red" }}>{error}</p></div>;

  return (
    <div className="admin-content">
      <h1 className="admin-title">Gestión de Combos</h1>

      <button className="btn-crear" onClick={handleOpenCreate}>+ Crear Combo</button>
      ...
      <div className="table-container">
        <table className="tabla-productos">
          ...
          <tbody>
            {productos.length === 0 ? (
              <tr><td colSpan="7" style={{ textAlign: "center", padding: "20px" }}>No hay combos registrados</td></tr>
            ) : (
              productos.map((p) => (
                <tr key={p.idItem}>
                  <td>{p.idItem}</td>
